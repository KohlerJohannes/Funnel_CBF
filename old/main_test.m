...
%% funnel
tspan = [0 2];
params.psi=1e-3*[1;1e1];%constant funnel for now; position, then velocity
params.barrier_min=1e-10;
%params.sigma=params.psi*0.9;%sigma factor related to derivative
%params.delta=1;%this needs to change if funnel contracts faster
%params.nu=params.sigma*params.delta;%
params.q_0=q0;
...
params.funnel_option=1;%0:PD; 1:funnel, 2:funnel-QP
params.CBF=true;
...
function [dx,tau,q_des,dq_des,psi,barrier,lambda,tau_ref] = robotDynamics(t, x, robot,params)
    n=length(x)/2;%n = numel(robot.homeConfiguration);  % Number of DOFs
    q = x(1:n);
    dq = x(n+1:end);
    t
    % Compute dynamics terms
    M = eye(n);%massMatrix(robot, q);
    C = eye(n)*dq;%velocityProduct(robot, q, dq);   % C(q,dq)*dq
    G = eye(n)*q;%gravityTorque(robot, q);
...
%linear interpolation from start to end in time
 n = 3%numel(robot.homeConfiguration);  % Number of DOFs
...
k=params.funnel_gain;%desired decay rate
...
b=barrier
tau_0=-beta_2/b;
tau_ref=zeros(3,1);%gravityTorque(robot, q)+ones(length(q),1)*sin(params.omega_ref*t)*params.tau_ref;
%;
if params.CBF==true
